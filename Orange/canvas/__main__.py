...
def main(argv=None):
    ...
    # canvas_window.open_scheme_file('/Users/didi/Documents/hegsns/hegsns/TODS/tods_Guanchu/new_pipeline.ows')  # load default pipeline
    # canvas_window.load_scheme('/Users/didi/Documents/hegsns/hegsns/TODS/tods_Guanchu/new_pipeline.ows')  # load default pipeline

    def load_new_pipeline_template(filename): # revised by Guanchu based on load_scheme() in canvasmain.py Line 1147
        try:
            with open(filename, "rb") as f:
                new_scheme = canvas_window.new_scheme_from_contents_and_path(f, filename)
                canvas_window.set_new_scheme(new_scheme)

            log.info("Load pipeline from %s.", filename)
            app.fileOpenRequest.connect(canvas_window.open_scheme_file)
        except:
            log.info("Miss pipeline template in %s, start from blank canvas.", filename)
            return

    current_path = os.path.dirname(__file__)
    default_pipeline_path = os.path.join(current_path, 'workflows/.tods_default.ows')
    load_new_pipeline_template(default_pipeline_path)

    ##################################################

    # print('open_scheme_file:', canvas_window.open_scheme_file)
