...
from tods.timeseries_processing.MovingAverageTransformer import *

class OWFilterBlur(SingleInputWidget):
    name = "FilterBlur"
    description = ("FilterBlur")
    ...
    with_mean = Setting(True)
    with_std = Setting(True)
    ...
    primitive = MovingAverageTransformerPrimitive
    ...
    def _init_ui(self):
        ...
        gui.checkBox(box, self, "with_mean", label='WindowSize3',  callback=None)
        gui.checkBox(box, self, "with_std", label='l2 Norm',  callback=None)
...
if __name__ == "__main__":
    WidgetPreview(OWFilterBlur).run(Orange.data.Table("iris"))
