...
from tods.timeseries_processing.SKStandardScaler import *

class OWRotate(SingleInputWidget):
    name = "Rotate"
    description = ("Rotate.")
    ...
    primitive = SKStandardScalerPrimitive
    ...
    def _init_ui(self):
        ...
        gui.checkBox(box, self, "with_mean", label='Center the data before scaling.',  callback=None)
        gui.checkBox(box, self, "with_std", label='Scale the data to unit variance.',  callback=None)
...
if __name__ == "__main__":
    WidgetPreview(OWRotate).run(Orange.data.Table("iris"))
