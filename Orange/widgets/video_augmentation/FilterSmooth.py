...
from tods.timeseries_processing.SKPowerTransformer import *

class OWFilterSmooth(SingleInputWidget):
    name = "FilterSmooth"
    description = ("FilterSmooth.")
    icon = "icons/power_transformation.svg"
    ...
    method = Setting('yeo-johnson')
    standardize = Setting(True)
    ...
    primitive = SKPowerTransformerPrimitive
    ...
    def _init_ui(self):
        ...
        gui.comboBox(box, self, 'method',
                     label='Algorithm for Power Transformation.',
                     items=['yeo-johnson', 'box-cox'], callback=None)

        gui.checkBox(box, self, "standardize", label='Scale the data to zero mean and unit variance.',  callback=None)
...
if __name__ == "__main__":
    WidgetPreview(OWFilterSmooth).run(Orange.data.Table("iris"))
