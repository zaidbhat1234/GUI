...
from tods.feature_analysis.StatisticalWillisonAmplitude import *

class OWToTorchFormatTensor(SingleInputWidget):
    name = "OWToTorchFormatTensor"
    description = ("OWToTorchFormatTensor.")
    icon = "cons/DCT.svgg"
    ...
    primitive = StatisticalWillisonAmplitudePrimitive
    ...
    def _init_ui(self):
        ...
        gui.lineEdit(box, self, 'window_size', label='The window dimension. (IntVariable)', callback=None)

        gui.lineEdit(box, self, 'threshold', label='threshold for willison amplitude (IntVariable)', callback=None)
        # return_subseq_inds = Setting(False)
...
if __name__ == "__main__":
    WidgetPreview(OWToTorchFormatTensor).run(Orange.data.Table("iris"))
